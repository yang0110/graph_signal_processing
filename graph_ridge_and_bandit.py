...
user_f, _=datasets.make_blobs(n_samples=user_num, n_features=dimension, centers=5, cluster_std=3, shuffle=False, random_state=2019)
...
plt.pcolor(adj)
plt.colorbar()
plt.savefig(path+'adj_user_num_cluster_std_%s_%s'%(user_num, cluster_std)+'.png', dpi=200)
plt.show()
...
plt.savefig(path+'random_model/'+'LinUCB_vs_LapUCB_vs_LamUCB_cum_regret_user_num_noise_cluster_%s_%s_%s'%(user_num, noise_level, cluster_std)+'.png', dpi=100)
plt.show()
...
plt.legend(loc=0, fontsize=12)
plt.savefig(path+'random_model/'+'LinUCB_vs_LapUCB_vs_LamUCB_learning_error_user_num_noise_cluster_%s_%s_%s'%(user_num, noise_level, cluster_std)+'.png', dpi=100)
plt.show()
