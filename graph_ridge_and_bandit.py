...
####
linucb=LinUCB(user_num, item_num, dimension, pool_size, user_f, item_f,noisy_signal, 1.0, confidence_inter)
l_cum_regret, l_error_list, l_e_array=linucb.run(user_array, item_array, iteration)


graph_ridge=Graph_ridge(user_num, item_num, dimension, lap, adj, pool_size, user_f, item_f, noisy_signal, 0.5, confidence_inter)
...
plt.savefig(path+'random_model/'+'cum_regret_user_num_noise_thrs_%s_%s_%s'%(user_num, noise_level, thrs)+'.png', dpi=100)
plt.show()
...
plt.xlabel('Iteration', fontsize=12)
plt.ylabel('Learning Error (Frobenious Norm)', fontsize=12)
plt.title('User num=%s, noise=%s, T=%s'%(user_num, noise_level, thrs), fontsize=12)
plt.legend(loc=0, fontsize=12)
plt.savefig(path+'random_model/'+'learing_error_user_num_noise_thrs_%s_%s_%s'%(user_num, noise_level, thrs)+'.png', dpi=100)
plt.show()
...
plt.xlabel('iteration', fontsize=12)
plt.ylabel('Cum regret', fontsize=12)
plt.title('LinUCB \n Noise= %s, T= %s'%(noise_level, thrs), fontsize=12)
plt.savefig(path+'random_model/'+'LinUCB-top-5_users_noise_%s_threshold_%s'%(noise_level, thrs)+'.png', dpi=100)
plt.show()
...
plt.ylabel('Cum regret', fontsize=12)
plt.title('LapUCB \n Noise= %s, T= %s'%(noise_level, thrs), fontsize=12)
plt.savefig(path+'random_model/'+'LapUCB-top-5_users_noise_%s_threshold_%s'%(noise_level, thrs)+'.png', dpi=100)
plt.show()
